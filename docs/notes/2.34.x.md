...
Bumped the default Coursier version to `v2.1.25-M26` to fix a bug where transitive dependencies were incorrectly omitted from lockfiles when resolving non-default versions of intermediate dependencies. Additionally, ARM64 binaries (macOS and Linux) are now downloaded directly from the official Coursier release channel rather than the VirtusLab fallback.
