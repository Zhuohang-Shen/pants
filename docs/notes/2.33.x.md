...
Fixed a bug where pants --changed-since would unnecessarily invalidate all targets in a BUILD file when only whitespace or unrelated lines were modified (requires --enable-target-origin-sources-blocks=True).
