...
async def resolve_dependencies_multi(
    ...
    relevant_inference_request_types_per_tgt = [
        [
            inference_request_type
    ...
    # TODO: If an inference request type can support multiple source files, do that here.
    inferred_per_tgt: tuple[tuple[InferredDependencies, ...], ...] = await concurrently(
        concurrently(
            infer_dependencies(
                **implicitly(
                    {
        ...
        for tgt, relevant_inference_request_types in zip(
            included_tgts, relevant_inference_request_types_per_tgt
        )
    )
