...
async def load_lockfile(
    ...
    try:
        ...
        if python_setup.invalid_lockfile_behavior == InvalidLockfileBehavior.ignore:
            # Forget the metadata, since we don't want to use it for validation.
            metadata = None
    except (IntrinsicError, FileNotFoundError, InvalidLockfileError):
        # No metadata file or resource found, or the metadata version is unknown, so fall through
        # to finding a metadata header block prepended to the lockfile itself.
        pass
