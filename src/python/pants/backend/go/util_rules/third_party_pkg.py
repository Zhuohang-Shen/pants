...
@dataclass(frozen=True)
class DeduplicatedModuleRequest:
...
def _extract_go_sum_entries_for_module(
    ...
    We return them verbatim so that a synthetic go.sum can be reconstructed in
    a deduplicated sandbox.
    """
    prefix_content = f"{name} {version} "
    prefix_gomod = f"{name} {version}/go.mod "
    lines: list[str] = []
    for line in go_sum_content.decode("utf-8").splitlines():
        if line.startswith(prefix_content) or line.startswith(prefix_gomod):
            lines.append(line)
    return tuple(lines)
...
@rule
async def download_and_analyze_module_deduped(
    request: DeduplicatedModuleRequest,
    analyzer: PackageAnalyzerSetup,
) -> AnalyzedThirdPartyModule:
    """Download and analyze a module using a synthetic go.mod for deduplication.

    This rule has the same behavior as analyze_go_third_party_module but is keyed
    by (name, version, minimum_go_version, build_opts, go_sum_entries), enabling
    the Pants engine to deduplicate identical module downloads across go.mods.
    ...
    from the consuming go.mod's real go.sum (see DeduplicatedModuleRequest for
    the full argument for why this is safe).
    """
...
async def download_and_analyze_third_party_packages(
    ...
    # Read the real go.sum once so we can pass per-module entries into the
    # deduplicated download path. This keeps Go's checksum verification intact
    # while still allowing the engine to dedupe identical module@version
    # downloads across different go.mods.
    ...
    # Use the deduplicated path: keyed by (name, version, minimum_go_version,
    # build_opts, go_sum_entries). The Pants engine will automatically
    # deduplicate identical requests across different go.mods, reducing
    # downloads from O(N*M) to O(M).
    analyzed_modules = await concurrently(
        download_and_analyze_module_deduped(
            DeduplicatedModuleRequest(
                name=mod.name,
                version=mod.version,
                minimum_go_version=mod.minimum_go_version,
                build_opts=request.build_opts,
                go_sum_entries=_extract_go_sum_entries_for_module(
                    go_sum_content, mod.name, mod.version
                ),
            ),
            **implicitly(),
        )
