...
def python_pb2_include_imports(content: bytes) -> bool:
    """True if `buf.gen.yaml`'s `buf.build/protocolbuffers/python` plugin has
    `include_imports: true` set, meaning buf will materialize `*_pb2.py` files
    for transitively-imported BSR-dep protos into the generated digest."""
    try:
        data = yaml.safe_load(content)
    except yaml.YAMLError:
        return False
    ...
    for plugin in plugins:
        ...
        if not isinstance(remote, str):
            continue
        if remote.split(":", 1)[0] != "buf.build/protocolbuffers/python":
            continue
        if plugin.get("include_imports") is True:
            return True
    return False
...
def synthesize_pinned_buf_gen_yaml(
    ...
    *,
    ...
    defaults. `extra_pins` values are formatted as `"vX.Y:N"` (string, parsed
    here) for parity with Pants's other dict options. `protoc_builtin:` and
    `local:` plugins are not subject to pin enforcement.
    """
    try:
        data = yaml.safe_load(content)
    except yaml.YAMLError:
    ...
    if unresolvable:
        ...
        raise UnpinnedBufPluginError(
            ...
            "For the plugins in Pants's built-in registry, leave the entry without "
            "any pin and Pants will fill in a default version+revision. Otherwise, "
...
def check_pinned_remote_plugins(
    ...
    *,
    ...
    """Raise if `remote:` plugin entries can't be resolved to a full pin.

    Wraps `synthesize_pinned_buf_gen_yaml` for callers that only want to validate
    (e.g. inference) without consuming the synthesized content. Codegen rules
    should call `synthesize_pinned_buf_gen_yaml` and use its output instead.
    """
