...
def test_python_pb2_include_imports_true() -> None:
    content = dedent(
        """\
        version: v2
        plugins:
    ...
    assert python_pb2_include_imports(content) is True


def test_python_pb2_include_imports_false_when_unset() -> None:
    content = dedent(
        """\
        version: v2
        plugins:
    ...
    assert python_pb2_include_imports(content) is False


def test_python_pb2_include_imports_only_checks_python_plugin() -> None:
    """`include_imports` on a non-python plugin doesn't count."""
    content = dedent(
        """\
        version: v2
        plugins:
...
def test_python_pb2_include_imports_tolerates_pinned_remote() -> None:
    content = dedent(
        """\
        version: v2
        plugins:
