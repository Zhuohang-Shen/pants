...
# when buf runs with `include_imports: true` on the protocolbuffers/python
# plugin. The buf module mapper uses this to register dep-inference owners for
# imports of BSR-provided modules from hand-written user code. To refresh:
...
async def infer_dependencies(
    ...
    # Protoc path: gated on `grpc=True` and the subsystem booleans (existing
    # behavior — Pants is the one driving protoc invocation).
