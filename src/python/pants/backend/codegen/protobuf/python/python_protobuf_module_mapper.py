...
# Suffixes relevant to Python codegen. Order matters only inasmuch as `_pb2` is
# always registered when its plugin is matched; the service suffixes register iff
# their matching plugin appears in `buf.gen.yaml`.
_PB2_SUFFIX = "_pb2"
...
def _fallback_plan(target: Target) -> _BufStripPlan:
    ...
    template, so we register only `_pb2`. Same convention as before."""
    proto_path = target[ProtobufSourceField].file_path
...
async def map_protobuf_to_python_modules(
    ...
    # `buf.gen.yaml` has `include_imports: true` on
    # `buf.build/protocolbuffers/python` — otherwise the file isn't in the
    # `GeneratedSources` digest and registering ownership would lie. Targets
    # without `include_imports` are skipped; users either set `include_imports`
    # or accept the dep-inference warning.
    if buf_layout.deps and buf_targets:
        ...
        for tgt, gen in zip(buf_targets, buf_gen_contents):
            if not python_pb2_include_imports(gen.content):
                continue
            resolve = tgt[PythonResolveField].normalized_value(python_setup)
