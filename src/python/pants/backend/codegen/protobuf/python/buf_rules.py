...
class GeneratePythonFromProtobufViaBufRequest:
    """Buf-based code-generation request, dispatched to from
    `generate_python_from_protobuf` when the target opts in via
...
async def generate_python_from_protobuf_via_buf(
    ...
    # latest on the BSR. Mirrors how unpinned `remote:` plugins are rejected.
    config_yaml_paths = [
